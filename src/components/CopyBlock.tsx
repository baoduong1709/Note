...
export default function CopyBlock({ code, language = "bash", noteId, editable = false, onChange, triggerToast }: CopyBlockProps) {
  ...
  const getFinalCommand = (): string => {
    ...
    Object.entries(variables).forEach(([key, val]) => {
      final = final.replaceAll(`{{${key}}}`, val || `{{${key}}}`);
    });
    return final;
  };
  ...
  return (
    <div className="p-3 rounded-lg bg-zinc-900 border border-white/5 space-y-2 my-1 relative group">
      {/* Block Header */}
        ...
        <div className="flex items-center gap-2">
          {vars.length > 0 && (
            <button 
              ...
              className="text-[9px] text-purple-400 hover:text-purple-300 flex items-center gap-0.5 font-medium transition-all"
            >
      ...
      {/* Code display */}
      {editable ? (
        <div className="bg-black/30 p-1 rounded font-mono text-xs text-purple-300 overflow-hidden">
          <textarea
            ref={textareaRef}
            value={code}
            onChange={(e) => {
            ...
            className="w-full bg-transparent border-none outline-none font-mono text-xs text-purple-300 focus:ring-0 p-1.5 resize-none min-h-[28px] overflow-hidden leading-normal"
            rows={1}
          />
        </div>
      ) : (
        <div className="bg-black/30 p-2.5 rounded font-mono text-xs text-purple-300 break-all overflow-x-auto">
          <code>{code}</code>
        </div>
      )}
      ...
      {vars.length > 0 && (
        <div className="flex flex-wrap gap-2 pt-1 border-t border-white/5">
          {vars.map(v => (
            <div key={v} className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded text-[10px]">
              ...
              <input 
                ...
                className="bg-transparent border-none outline-none text-purple-300 w-16 text-[10px] focus:ring-0 p-0"
              />
            </div>
          ))}
        </div>
      ...
      {showVarModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-panel w-full max-w-[460px] rounded-xl p-5 shadow-2xl space-y-4">
            <div className="flex items-center gap-2.5 text-purple-400 border-b border-white/5 pb-2 shrink-0">
            ...
            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-[9px] text-zinc-500 font-bold uppercase mb-1">Cấu trúc gốc</label>
                <code className="text-[10px] font-mono text-zinc-400 bg-zinc-950/60 p-2 rounded block break-all">{code}</code>
              </div>

              <div className="grid grid-cols-2 gap-3">
                {vars.map(v => (
                  <div key={v}>
                    <label className="block text-zinc-400 font-medium mb-1">{v}</label>
                    <input 
                      type="text" 
                      value={variables[v] || ""} 
                      onChange={(e) => handleVariableChange(v, e.target.value)}
                      className="w-full p-2 rounded glass-input text-zinc-300 text-xs"
                      placeholder={`Nhập ${v}...`}
                    />
                  </div>
                ))}
              ...
              <div>
                <label className="block text-[9px] text-zinc-500 font-bold uppercase mb-1">Preview lệnh copy cuối cùng</label>
                <code className="text-[11px] font-mono text-teal-300 bg-black/40 p-2.5 rounded block break-all">
                  {getFinalCommand()}
                </code>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2.5 shrink-0 pt-2">
              <button 
                ...
                className="px-3.5 py-1.5 rounded-md text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-semibold transition-all"
              >
                Đóng
              </button>
              <button 
      ...
      {showDangerModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-panel w-full max-w-[420px] border border-red-500/20 rounded-xl p-5 shadow-2xl space-y-4">
            <div className="flex items-center gap-2.5 text-red-400 mb-2 shrink-0">
            ...
            <div className="bg-black/60 p-2.5 rounded-lg font-mono text-[11px] text-red-300 break-all">
