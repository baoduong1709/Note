  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="glass-panel w-full max-w-[460px] border border-blue-500/20 rounded-xl p-5 shadow-2xl space-y-4">
        {/* Modal Header */}
        <div className="flex items-center gap-2.5 text-blue-400 border-b border-white/5 pb-3 shrink-0">
        ...
        <div className="space-y-3 text-xs text-zinc-300">
          ...
          <div className="space-y-1.5 bg-zinc-950/60 p-3 rounded border border-white/5 font-mono text-[10px] leading-relaxed">
            <p className="text-purple-400 font-bold mb-1">Actions to Execute:</p>
            {actionDetails.map((act, index) => (
              <p key={index} className="text-zinc-300">
                {act.label}: {act.oldVal ? `${act.oldVal} → ` : ""}<span className="text-teal-400 font-semibold">{act.newVal}</span>
              </p>
            ))}
          </div>

          {/* Warning Banner */}
          <div className="flex items-start gap-2 bg-blue-950/15 border border-blue-900/20 p-2.5 rounded text-[10px] text-blue-400 leading-relaxed">
