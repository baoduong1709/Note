...
export default function RichTextEditor({
      ...
      if (initialContent.trim().startsWith("<") && initialContent.trim().endsWith(">")) {
        ...
        const paragraphs = initialContent
          ...
          .map(line => {
            ...
            // Basic title parser
            if (line.startsWith("# ")) return `<h2 class="text-sm font-bold text-teal-600 dark:text-teal-400 mt-3 mb-1.5 border-b border-zinc-200 dark:border-white/5 pb-1">${line.substring(2)}</h2>`;
            // Basic checkbox parser
            if (line.startsWith("- [ ]") || line.startsWith("- [x]")) {
              const checked = line.startsWith("- [x]");
              const text = line.substring(5).trim();
              return `<div class="flex items-center gap-2 py-0.5"><input type="checkbox" ${checked ? "checked" : ""} class="todo-chk rounded border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-purple-600 focus:ring-purple-600 cursor-pointer w-4 h-4 shrink-0" /> <span class="text-xs ${checked ? "line-through text-zinc-500" : "text-zinc-700 dark:text-zinc-300"}">${text}</span></div>`;
            }
            return `<p class="text-xs text-zinc-700 dark:text-zinc-300 leading-relaxed my-1">${line}</p>`;
          })
          .join("");
  ...
  const insertCopyBlock = () => {
    ...
    const html = 
      `<div class="copy-block-embed border border-zinc-200 dark:border-white/5 bg-zinc-100 dark:bg-zinc-900 rounded-lg p-3 my-2 space-y-2 relative" contenteditable="false" id="${blockId}">` +
        `<div class="flex justify-between items-center text-[9px] font-mono text-zinc-500 dark:text-zinc-400">` +
        ...
        `</div>` +
        `<div class="bg-black/5 dark:bg-black/30 p-1 rounded font-mono text-xs text-purple-750 dark:text-purple-300 overflow-hidden">` +
          `<textarea class="code-input w-full bg-transparent border-none outline-none font-mono text-xs text-zinc-700 dark:text-purple-300 focus:ring-0 p-1.5 resize-none h-12 leading-normal" placeholder="Nhập câu lệnh..."></textarea>` +
        `</div>` +
      `</div><p><br></p>`;
  ...
  const insertChecklist = () => {
    const html = `<div class="flex items-center gap-2 py-0.5"><input type="checkbox" class="todo-chk rounded border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-purple-600 focus:ring-purple-600 cursor-pointer w-4 h-4 shrink-0" /> <span class="text-xs text-zinc-700 dark:text-zinc-300">Việc cần làm...</span></div>`;
    editorRef.current?.focus();
    document.execCommand("insertHTML", false, html);
    if (editorRef.current) {
      triggerAutoSave(title, editorRef.current.innerHTML);
